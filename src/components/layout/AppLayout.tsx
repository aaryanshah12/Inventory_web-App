...
export default function AppLayout({ children }: { children: React.ReactNode }) {
  ...
  const SidebarContent = () => (
    <div className="flex flex-col h-full">
        ...
        <div className="px-4 py-3 border-t border-border">
          ...
          {profile.factories.slice(0, 3).map((f: any) => (
            <div key={f.id} className="flex items-center gap-2 py-1">
              ...
              <span className="text-xs text-primary-70 truncate">{f.name}</span>
            </div>
          ))}
        </div>
      )}
      ...
      <div className="px-4 py-4 border-t border-border space-y-1">
        <button
          onClick={toggleTheme}
          className="flex items-center gap-2 w-full px-3 py-2 rounded-lg text-sm text-muted hover:text-primary hover:bg-layer transition-all"
        >
          {theme === 'dark'
            ? <><Sun size={16} className="text-owner" /> Light Mode</>
            : <><Moon size={16} className="text-inputer" /> Dark Mode</>
          }
  ...
  return (
    <div className="flex min-h-screen">
      ...
      <aside
        id="sidebar"
        className="hidden lg:flex w-64 flex-shrink-0 bg-panel border-r border-border flex-col fixed top-0 left-0 h-screen z-30"
      >
        <SidebarContent />
      </aside>
      ...
      {sidebarOpen && (
        <div className="fixed inset-0 z-40 lg:hidden">
          ...
          <aside
            id="sidebar"
            className="absolute top-0 left-0 h-full w-72 bg-panel border-r border-border flex flex-col z-50 animate-fade-up"
          >
            <SidebarContent />
          </aside>
        </div>
