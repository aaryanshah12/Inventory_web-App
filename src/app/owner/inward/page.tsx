...
export default function OwnerInwardPage() {
  ...
  useEffect(() => {
    // when switching factory, reset product to first available option
    const firstId = productOptions[0]?.id ?? ''
  ...
  const handleDownload = () => {
    ...
    a.download = `Inward_${year}_${monthLabel}.csv`
    a.click()
  ...
  return (
    <AppLayout>
      <div className="p-4 md:p-8">
        <PageHeader
          title="Inward"
          subtitle="Owner · Log inward product quantities"
          accent="owner"
          actions={
            <div className="flex items-center gap-2">
              <button className="btn btn-owner-secondary gap-2" onClick={handleDownload} disabled={entries.length === 0}>
                <Download size={16} /> Download CSV
              </button>
              <button className="btn btn-owner gap-2" onClick={openModal}>
              ...
              <tbody>
                {entries.map(e => (
                  <tr key={e.id}>
                    ...
                    <td className="font-mono text-owner">{Number(e.tons ?? 0).toFixed(3)}</td>
              ...
              {entries.length > 0 && (
                <tfoot>
                  <tr>
                    ...
                    <td className="font-mono text-owner">{totals.totalTons.toFixed(3)}</td>
                    <td />
                  </tr>
                </tfoot>
              )}
          ...
          <div className="md:hidden p-4">
              ...
              {entries.map(e => (
                <div key={e.id} className="data-card">
                  ...
                  <div className="data-card-grid">
                    ...
                    <span className="font-mono text-owner text-right">{Number(e.tons ?? 0).toFixed(3)}</span>
                  </div>
            ...
            {entries.length > 0 && (
              <div className="data-card mt-3">
                ...
                <div className="data-card-grid">
                  <span className="data-card-label">TONS (in Kg)</span>
                  <span className="data-card-value text-owner font-mono text-right">{totals.totalTons.toFixed(3)}</span>
                </div>
              </div>
            )}
          </div>
      ...
      <SimpleModal
        ...
        subtitle="Select product, date, and tons"
      >
        ...
        <div className="flex justify-end gap-2">
          ...
          <button className="btn btn-owner gap-2" onClick={handleSave} disabled={saving || !hasTons}>
