...
export default function OwnerOutwardPage() {
  ...
  return (
    <AppLayout>
      <div className="p-4 md:p-8">
        <PageHeader
          title="Outward"
          subtitle="Owner · Log outward batches"
          accent="owner"
          actions={
              ...
              {entries.length > 0 && (
                <tfoot>
                  <tr>
                    ...
                    <td className="font-mono text-owner">{totals.totalReal.toFixed(2)}</td>
                    <td />
                  </tr>
                </tfoot>
              )}
            ...
            {entries.length > 0 && (
              <div className="data-card mt-3">
                ...
                <div className="data-card-grid">
                  ...
                  <span className="data-card-label">Real (Kg)</span>
                  <span className="data-card-value font-mono text-owner text-right">{totals.totalReal.toFixed(2)}</span>
                </div>
              </div>
            )}
          </div>
      ...
      >
        <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
          ...
          {hasProduct && (
            <div className="flex flex-col gap-1">
              ...
              <input className="input" type="date" value={String(form.entry_date ?? '')} onChange={e => setForm(f => ({ ...f, entry_date: e.target.value }))} />
            </div>
          )}
          ...
          {hasPurity && (
            <div className="flex flex-col gap-1">
              ...
              <input className="input" disabled value={Number(form.real ?? 0).toFixed(2)} />
            </div>
          )}
        </div>
        ...
        <div className="flex justify-end gap-2">
          ...
          <button className="btn btn-owner gap-2" onClick={handleSave} disabled={saving || !hasPurity}>
