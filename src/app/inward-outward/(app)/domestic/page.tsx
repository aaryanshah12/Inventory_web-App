...
export default function DomesticPage() {
  ...
  async function loadData() {
    setLoading(true)
    try {
      const [r, c, p] = await Promise.all([fetchDomestics(factoryId || undefined), fetchCompanies('customer'), fetchProducts()])
  ...
  function openEdit(row: IODomestic) { setEditing(row); setForm({ invoice_date: row.invoice_date, customer_id: row.customer_id ?? '', remarks: row.remarks ?? '', factory_id: row.factory_id ?? factoryId }); setItems(row.items?.length ? row.items.map(it => ({ ...it })) : [EMPTY_ITEM()]); setShowForm(true) }
  async function handleSave() {
    const validItems = items.filter(it => it.product_id)
    if (!validItems.length) { alert('Add at least one product.'); return }
    setSaving(true)
    try { await saveDomestic({ id: editing?.id, invoice_date: form.invoice_date, customer_id: form.customer_id || null, remarks: form.remarks || null, factory_id: form.factory_id || factoryId || null, items: validItems }); setShowForm(false); loadData() }
