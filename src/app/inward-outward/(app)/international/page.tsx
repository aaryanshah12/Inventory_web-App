...
import { fetchInternationals, saveInternational, deleteInternational, fetchCompanies, fetchProducts, fmtDate, today } from '@/lib/io/api'
...
export default function InternationalPage() {
  ...
  const [rows, setRows] = useState<IOInternational[]>([])
  ...
  async function loadData() {
    setLoading(true)
    try {
      const [r, c, p] = await Promise.all([fetchInternationals(factoryId || undefined), fetchCompanies('customer', factoryId || undefined), fetchProducts(factoryId || undefined)])
  ...
  function downloadTemplate() {
    ...
    const a = document.createElement('a'); a.href = url; a.download = 'international_template.csv'; a.click()
  ...
  async function handleImportConfirm() {
    ...
    try {
      for (const g of valid) {
        await saveInternational({ invoice_date: g.invoice_date, customer_id: g.customer_id || null, remarks: null, factory_id: g.factory_id || null, items: g.items.filter((it: any) => it.product_id).map((it: any) => ({ product_id: it.product_id, quantity: it.quantity, price: it.price, remarks: it.remarks })) })
      }
  ...
  async function handleExport() {
    ...
    const ExcelJS = (await import('exceljs')).default
    const wb = new ExcelJS.Workbook()
    const ws = wb.addWorksheet('International')
    ...
    const url = URL.createObjectURL(blob)
    const a = document.createElement('a'); a.href = url; a.download = 'international.xlsx'; a.click()
    URL.revokeObjectURL(url)
  }
  ...
  return (
    <div className="p-4 lg:p-6 max-w-7xl mx-auto">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 mb-5">
        <div><h1 className="text-xl font-bold text-primary">International</h1><p className="text-sm text-muted mt-0.5">{filtered.length} invoices</p></div>
        ...
        <div className="hidden sm:block overflow-x-auto">
          <table className="data-table">
            ...
            <tbody>
              ...
              : filtered.length === 0 ? <tr><td colSpan={7} className="py-12 text-center text-muted text-sm">No international invoices</td></tr>
              : filtered.map(row => (
                <tr key={row.id}>
        ...
        <div className="fixed inset-0 z-50 flex items-start justify-center bg-black/60 p-4 overflow-y-auto">
          <div className="rounded-2xl shadow-2xl w-full max-w-3xl my-6 border border-border" style={{ background: 'var(--color-panel)' }}>
            <div className="flex items-center justify-between px-6 py-4 border-b border-border">
              <h2 className="text-base font-bold text-primary">{editing ? 'Edit Invoice' : 'New International Invoice'}</h2>
            ...
            <div className="p-6 space-y-5">
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                ...
                <div className="sm:col-span-2">
                  ...
                  <select value={form.customer_id} onChange={e => setForm(f => ({ ...f, customer_id: e.target.value }))} className="input w-full"><option value="">— Select Customer —</option>{companies.map(c => <option key={c.id} value={c.id}>{c.company_name}</option>)}</select>
                </div>
      ...
      {importOpen && (
        <div className="fixed inset-0 z-50 flex items-start justify-center bg-black/60 p-4 overflow-y-auto">
          <div className="rounded-2xl shadow-2xl w-full max-w-4xl my-6 border border-border" style={{ background: 'var(--color-panel)' }}>
            <div className="flex items-center justify-between px-6 py-4 border-b border-border">
              <div>
                <h2 className="text-base font-bold text-primary">Import International Invoices</h2>
